...
def compute_homography(data, keep_k_points=1000, debug=False):
    ...
    real_warped_keypoints = warp_keypoints(m_keypoints, real_H)
    ...
    if debug:
        ...
        img_debug = np.ascontiguousarray((img_debug*127.5).astype(np.uint8))
        ...
        cv2.imshow("image_aug", img_debug)

        cv2.waitKey(1)
    return correctness1, correctness3, correctness5, M.sum()
